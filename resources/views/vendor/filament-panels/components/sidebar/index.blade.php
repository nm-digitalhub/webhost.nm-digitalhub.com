...
@php
    $openSidebarClasses = 'fi-sidebar-open w-[--sidebar-width] translate-x-0 shadow-xl ring-1 ring-gray-200 dark:ring-white/10 rtl:-translate-x-0';
...
<aside
    ...
    {{
        $attributes->class([
            'fi-sidebar fixed inset-y-0 start-0 z-30 flex flex-col h-screen content-start bg-white text-[#0D1E3C] transition-all dark:bg-gray-900 lg:z-0 lg:bg-transparent lg:shadow-none lg:ring-0 lg:transition-none dark:lg:bg-transparent font-heebo',
...
>
    <div class="overflow-x-clip">
        <header class="fi-sidebar-header flex h-16 items-center bg-white px-6 ring-1 ring-gray-100 dark:bg-gray-900 dark:ring-white/10 lg:shadow-sm">
            <div
            ...
            >
                @if ($homeUrl = filament()->getHomeUrl())
                    <a {{ \Filament\Support\generate_href_html($homeUrl) }}>
                        <img src="{{ asset('images/logo.svg') }}" class="w-10 h-10 rtl-flip" alt="Logo" />
                    </a>
                @else
                    <img src="{{ asset('images/logo.svg') }}" class="w-10 h-10 rtl-flip" alt="Logo" />
                @endif
    ...
    <nav class="fi-sidebar-nav flex-grow flex flex-col gap-y-7 overflow-y-auto overflow-x-hidden px-6 py-8" style="scrollbar-gutter: stable">
        ...
        @if (filament()->hasTenancy() && filament()->hasTenantMenu())
            <div
                @class(['fi-sidebar-nav-tenant-menu-ctn', '-mx-2' => ! filament()->isSidebarCollapsibleOnDesktop()])
                @if (filament()->isSidebarCollapsibleOnDesktop())
